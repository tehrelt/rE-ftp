...
export const DEntryDisplay = ({ dentry, doubleClickCallback }: Props) => {
  ...
  return (
    <div
      className="grid grid-cols-6 items-center border-b-4 border-slate-300 hover:bg-slate-300 px-10 py-2 transition-all ease-in-out select-none "
      onDoubleClick={() => doubleClickCallback(dentry.fileName)}>
      <svg
        xmlns="http://www.w3.org/2000/svg"
        viewBox="0 0 24 24"
        className="w-8 h-8 fill-gray-600 mx-auto"
      >
        {dentry.isDir ? (
      ...
      <div className="select-none"> {!dentry.isDir && dentry.size} </div>
